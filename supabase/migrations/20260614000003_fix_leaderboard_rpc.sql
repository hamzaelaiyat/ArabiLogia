-- Add points column for real-point scoring (each question = 1 point)
ALTER TABLE exam_results ADD COLUMN IF NOT EXISTS points INTEGER;

-- Backfill points for existing rows (computed from exam question count - wrong answers)
UPDATE exam_results er
SET points = (
  SELECT (jsonb_array_length(e.data->'q') - COALESCE(array_length(er.wrong_answers, 1), 0))::integer
  FROM exams e
  WHERE e.id = er.exam_id
)
WHERE er.points IS NULL;

-- Fix get_leaderboard_by_period: avoid Cartesian product and use points
...
-- Update leaderboard view to use points instead of percentage
...
-- Prevent duplicate completed submissions at the DB level
CREATE UNIQUE INDEX IF NOT EXISTS uniq_first_completion
ON exam_results (user_id, exam_id) WHERE status = 'completed';
