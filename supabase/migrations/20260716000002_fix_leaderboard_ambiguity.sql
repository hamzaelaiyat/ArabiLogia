-- Fix ambiguous column reference in get_leaderboard_by_period
...
BEGIN
    ...
    user_totals AS (
        SELECT bpe.user_id,
               ...
               + COALESCE(pa_sum.total_adjustments, 0::double precision)) AS total_score
        FROM best_per_exam bpe
        LEFT JOIN (
            SELECT pa.user_id, SUM(pa.amount)::double precision AS total_adjustments
            FROM public.points_adjustments pa
            GROUP BY pa.user_id
        ) pa_sum ON pa_sum.user_id = bpe.user_id
        GROUP BY bpe.user_id, pa_sum.total_adjustments
    ),
    user_averages AS (
        SELECT fr.user_id,
